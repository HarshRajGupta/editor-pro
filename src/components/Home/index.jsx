...
function Files({ user, setUser }) {
  ...
  const createFile = async () => {
    if (!fileRef.current.value) return toast.error("Please enter a file name");
    setCreating(true);
    toast.info("Creating file...");
    try {
      const DefaultValue = DefaultCode.find((x) => x.id === fileType.id)?.code;
          ...
          {files.length === 0 ? (
            ...
            <div className="grid max-h-full overflow-y-auto">
              {files.map((file, index) => (
                <Line key={index} file={file} index={index} />
              ))}
            </div>
          )}
        </div>
