...
const registrationMail = (email, name) => {
  ...
  try {
    mailTransporter.sendMail(mailDetails, function (err, data) {
      if (err) {
        console.log(
          `ERROR: while sending registration mail to ${email} in pipeline`,
        );
        console.error(err);
      } else {
...
const invitationMail = (userMail, newEmail, owner, fileName, docId) => {
  ...
  try {
    mailTransporter.sendMail(mailDetails, function (err, data) {
      if (err) {
        console.log(
          `ERROR: while sending invitation mail to ${newEmail} in pipeline`,
        );
        console.error(err);
      } else {
        console.log(`DEBUG: Mail sent to ${newEmail}`);
      }
    });
  } catch (err) {
    console.log(`ERROR: while sending invitation mail to ${newEmail}`);
    console.error(err);
  }
};

const serverRestart = () => {
  const subject = `Server Restart Completed on Editor-Pro at ${new Date().toLocaleString()} from ${process.env.NODE_ENV} Environment`;
