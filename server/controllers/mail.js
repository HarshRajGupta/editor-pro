...
const registrationMail = (email, name) => {
    const subject =
        "Welcome to Editor-Pro - Your Ultimate Document Collaboration Platform!";
...
const invitationMail = (userMail, newEmail, owner, fileName, docId) => {
    ...
    try {
        mailTransporter.sendMail(mailDetails, function (err, data) {
            if (err) {
                console.error(`ERROR: while sending invitation mail to ${newEmail} in pipeline`, err);
            }
        });
    } catch (err) {
        console.error(`ERROR: while sending invitation mail to ${newEmail}`, err);
    }
};
...
const serverRestart = () => {
    ...
    try {
        mailTransporter.sendMail(mailDetails, function (err, data) {
            if (err)
                console.error(`ERROR: while sending server restart mail in pipeline`, err);
        });
    } catch (err) {
        console.error(`ERROR: while sending server restart mail`, err);
    }
}
