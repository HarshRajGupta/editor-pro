...
const create = async (req, res) => {
    try {
        ...
        const user = req.user;
        if (!type || !name)
            return res.status(400).json({ success: false, message: "Please provide a valid type and name" });
...
const invite = async (req, res) => {
    try {
        ...
        if (invitedUser)
            await Connection.create({ documentId: document.id, userId: invitedUser.id, email });
        else
            await Connection.create({ documentId: document.id, email: email });
...
const updateType = async (req, res) => {
    try {
        const { type } = req.body;
        if (!type) return res.status(400).json({ success: false, message: "Please provide a valid type" });
        const document = req.document;
