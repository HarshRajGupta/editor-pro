...
const getDocumentById = async (req, res) => {
  ...
  try {
    ...
    if (!document)
      return res
        .status(400)
        .json({ success: false, message: "Document does not exist" });
    ...
    return res
      .status(200)
      .json({ success: true, message: "Document fetched", document: document });
  } catch (e) {
...
const addUser = async (req, res) => {
  ...
  try {
    ...
    if (permit)
      return res
        .status(400)
        .json({ success: true, message: "Invitation already sent" });
    ...
    return res
      .status(200)
      .json({ success: true, message: `${newEmail} Invited` });
  } catch (e) {
...
const deleteDocument = async (req, res) => {
  ...
  try {
    ...
    if (doc.owner != userEmail) {
      return res
        .status(400)
        .json({ success: false, message: "Only owner can delete files" });
    }
...
const changeType = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ success: true, message: "Document type changed" });
  } catch (e) {
...
const openToAll = async (req, res) => {
  ...
  try {
    ...
    const userEmail = req.body.userEmail.toLowerCase();
    if (!docId || status === null || !userEmail)
      return res.status(400).json({
        success: false,
        message: "Bad Request",
        status: status,
    ...
    if (!document)
      return res
        .status(400)
        .json({ success: false, message: "Document does not exist" });
    if (document.owner != userEmail)
      return res
        .status(400)
        .json({ success: false, message: "Only owner can change this" });
    document.openToAll = status;
    await document.save();
    console.log(`DEBUG: ${docId} openToAll changed to ${status}`);
    return res
      .status(200)
      .json({ success: true, message: "Document openToAll changed" });
  } catch (e) {
